...
int sys_execve(char *filename, char *argv[], char *envp[]) {
    ...
    DEBUGK("ELF type %d\n", ehdr->e_type);
    DEBUGK("ELF machine %d\n", ehdr->e_machine);
    DEBUGK("ELF entry 0x%p\n", ehdr->e_entry);
    DEBUGK("ELF ehsize %d %d\n", ehdr->e_ehsize, sizeof(Elf32_Ehdr));
    DEBUGK("ELF phoff %d\n", ehdr->e_phoff);
    DEBUGK("ELF phnum %d\n", ehdr->e_phnum);
    DEBUGK("ELF phsize %d %d\n", ehdr->e_phentsize, sizeof(Elf32_Phdr));
    DEBUGK("ELF shoff %d\n", ehdr->e_shoff);
    DEBUGK("ELF shnum %d\n", ehdr->e_shnum);
    DEBUGK("ELF shsize %d %d\n", ehdr->e_shentsize, sizeof(Elf32_Shdr));

    // 段头
    Elf32_Phdr *phdr = (Elf32_Phdr *)alloc_kpage(1);
    lseek(fd, ehdr->e_phoff, SEEK_SET);
    read(fd, (char *)phdr, ehdr->e_phentsize * ehdr->e_phnum);
    DEBUGK("ELF segment size mem %d\n", ehdr->e_phentsize * ehdr->e_phnum);
    ...
    for (size_t i = 0; i < ehdr->e_phnum; i++) {
        ...
        DEBUGK("segment vaddr 0x%p paddr 0x%p\n", ptr->p_vaddr, ptr->p_paddr);
        ptr++;
    }
    ...
    for (size_t i = 0; i < ehdr->e_shnum; i++) {
        ...
        DEBUGK("section %s size %d vaddr 0x%p\n", sname, sptr->sh_size,
               sptr->sh_addr);
    ...
    for (size_t i = 0; i < encount; i++) {
        DEBUGK("v: 0x%X size: %d sec: %d b: %d t: %d n: %s\n", symptr->st_value,
               symptr->st_size, symptr->st_shndx,
               ELF32_ST_BIND(symptr->st_info), ELF32_ST_TYPE(symptr->st_info),
